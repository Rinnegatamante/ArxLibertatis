...
IntType Random::get(IntType min, IntType max) {
    arx_assert_msg(boost::is_integral<IntType>::value, "get must be called with ints");
...
RealType Random::getf(RealType min, RealType max) {
    arx_assert_msg(boost::is_float<RealType>::value, "getf must be called with floats");
